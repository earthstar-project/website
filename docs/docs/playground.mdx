...
> The next part uses a syncing API that is not yet publicly available.
...
```js
let otherReplica = new Earthstar.StorageAsync(shareAddress, Earthstar.FormatValidatorEs4, driver);
```
...
```js
let otherPeer = new Peer();
otherPeer.addStorage(otherReplica);
```

Now that we have our two peers, let's sync them:

```js
const syncer = new Earthstar.SyncerLocal(myPeer);
await syncer.addPeer(otherPeer);
```
