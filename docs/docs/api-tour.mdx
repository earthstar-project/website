...
A local replica of a share's documents. Provides operations for querying and setting documents. [Complete API documentation here.](https://doc.deno.land/https://deno.land/x/earthstar@v7.0.0/mod.ts/~/Replica)
...
It's possible to create your own replica drivers which conform to `IReplicaDriver`. [See the details here.](https://doc.deno.land/https://deno.land/x/earthstar@v7.0.0/mod.ts/~/IReplicaDriver)

## Validators

<img src="/api-tour/validator.svg" class="bg-gray-200 p-4"/>

Earthstar shares are able to store documents of different formats. But most of the time, you'll want to use the built-in one from this library: `es.4`. As such, Earthstar currenly only offers one format validator: `FormatValidatorEs4` ([API documentation here.](https://doc.deno.land/https://deno.land/x/earthstar@v7.0.0/mod.ts/~/FormatValidatorEs4))
...
It's possible to write your own validator which adheres to [`IFormatValidator`](https://doc.deno.land/https://deno.land/x/earthstar@v7.0.0/mod.ts/~/IFormatValidator).

## Identity

<img src="/api-tour/identity.svg" class="bg-gray-200 p-4"/>

An identity is a keypair containing a public address and a secret. This keypair is used to sign documents whenever they're written. The public address can be freely distributed, but the secret should be, well, secret. [API docs here](https://doc.deno.land/https://deno.land/x/earthstar@v7.0.0/mod.ts/~/AuthorKeypair).
...
Used to describe a document query. Used with `ReplicaAsync.query` and `QueryFollower`. [Full API details here](https://doc.deno.land/https://deno.land/x/earthstar@v7.0.0/mod.ts/~/Query.

## Doc

<img src="/api-tour/doc.svg" class="bg-gray-200 p-4"/>

Represents some cryptographically-signed user data. The contents of a `Doc` can only be changed via `ReplicaAsync`. A document cannot be moved from one share to another. [Full API documentation here](https://doc.deno.land/https://deno.land/x/earthstar@v7.0.0/mod.ts/~/Doc).

## Crypto

<img src="/api-tour/crypto.svg" class="bg-gray-200 p-4"/>

`Crypto` ([full API docs](https://doc.deno.land/https://deno.land/x/earthstar@v7.0.0/mod.ts/~/Crypto)) is used internally for signing and verifying documents using [ed25519](https://ed25519.cr.yp.to). In most cases, you will want to use it to generate new identity keypairs:
