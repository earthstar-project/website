...
We've designed Earthstar so that it cannot disclose which shares peers hold unless the the other peer knows about those shares itself, using something called a _zero knowledge proof_.
...
- [A brief introduction to `earthstar`, the Javascript reference implementation](/docs/browser-playground)
