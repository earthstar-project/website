...
**Share** — A collection of documents. Shares are identified by a **share address**. Shares are separate, unrelated worlds of data. Each document exists within exactly one share.
...
The file extension MUST be positioned at the end of the path to indicate a document with attachments:
