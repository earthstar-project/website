...
* The procedures needed for running a single local replica: signing, ingesting, and verifying documents; ingesting a verifying attachments.
...
**Peer** — A device which holds Earthstar data and wishes to sync with other peers. Peers may be individual users' devices and/or replica servers. A peer may hold data from multiple shares.
...
A peer MAY hold data from many shares. Each share's data is treated independently. Each document within a share is also independent; they don't form a chain or feed or refer to each other (e.g. Merkle backlinks).
...
Each document in a share exists at a path. For each path, Earthstar keeps the newest document from each identity who has ever written to that path.

In this example, the `/wiki/shared/Flowers` path contains 3 documents, because 3 different identities have written there. They may have written there hundreds of times, but we only keep the newest document from each identity, in that path.
...
When a new document arrives and an existing one is already there (from the same identity and same path), the new document overwrites the old one. Earthstar libraries MUST actually delete the older, overwritten document. The author's intent is to remove the old data.
...
Newly arrived attachments are only saved if a corresponding document is known, the peer does not yet have the attachment saved, and the attachment's size and hash match the size and hash on the document.
...
```
Valid path for document with attachment:

/images/squirrel.png

Invalid path for document with attachment:
/images.png/squirrel
```
...
A document MUST be **valid** in order to be ingested into a replica, either from a local write or a sync.
...
- `text` is a string holding utf-8 data
...
Peers SHOULD talk to each other over an encrypted connection such as HTTPS.
