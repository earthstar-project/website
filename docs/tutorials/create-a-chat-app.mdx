...
```js
import * as Earthstar from "https://FINAL_CDN_URL";
...
```typescript
import * as Earthstar from "./earthstar.js";
...
With these values we can instantiate an Earthstar Replica and write data to it.
...
Our chat application works, but it only allows us to talk to ourselves. We can enable online communication by adding three lines to the end of our script:
