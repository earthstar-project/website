...
As a fist step, replica servers tend to be the bottlenecks in the network so they might be a natural place to apply blocklists.  Apps should also provide users with the ability to hide other users.
...
At this stage Earthstar is of use to developers interested in building something with it. Apps and utilities for everyone else are coming soon.

If you'd like to start building a tool, application, or even an implementation of Earthstar:

- [Read the docs for `earthstar`, the Javascript reference implementation](/api/earthstar)
