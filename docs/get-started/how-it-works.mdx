...
Every document has a signature to help Earthstar verify its authorship. So if a document claims it was written by `@suzy`, you can be confident it was really written by `@suzy` and not alteredy by anyone else. These signatures are created using **identities**.
